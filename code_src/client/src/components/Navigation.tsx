...
export default function Navigation() {
  ...
  useEffect(() => { setCurrent(window.location.pathname) }, [])
  return <nav className="nav"><a className="brand" href="/">个人<span>星球</span></a><div className={open ? 'links open' : 'links'}>{links.map((link) => <a className={current === link.href ? 'active' : ''} aria-current={current === link.href ? 'page' : undefined} href={link.href} key={link.href} onClick={() => setOpen(false)}>{link.label}</a>)}</div><div className="nav-actions"><button aria-label="切换主题" className="icon-button" data-theme-toggle onClick={() => setDark(!dark)}>{dark ? <Sun size={18} /> : <Moon size={18} />}</button><button aria-label="打开菜单" className="icon-button mobile" onClick={() => setOpen(!open)}>{open ? <X size={18} /> : <Menu size={18} />}</button></div></nav>
}
