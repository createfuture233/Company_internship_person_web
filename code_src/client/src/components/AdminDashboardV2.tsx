...
const cards = [
  { key: "articles", label: "文章总数", icon: FileText, href: "/admin/articles", tone: "orange" },
  { key: "projects", label: "作品总数", icon: FolderKanban, href: "/admin/projects", tone: "green" },
  { key: "comments", label: "可见评论", icon: MessageCircle, href: "/admin/comments", tone: "cyan" },
  { key: "unreadMessages", label: "未读联系", icon: Mail, href: "/admin/messages", tone: "pink" },
] as const;
  ...
  return <section className="admin-module admin-overview">
    ...
    <div className="admin-stats overview-stats">
      {cards.map(({ key, label, icon: Icon, href, tone }) => (
        <a href={href} key={key} className={`overview-stat-${tone}`}>
          <Icon size={21} />
    ...
    <div className="overview-grid">
      <article className="overview-card overview-card-large">
        ...
        <div className="overview-chart-row">
          <DonutChart segments={contentStatus} />
          <div><MiniBars items={contentStatus} /><p>发布率 {data.insights.publishedRate}% · 草稿占比 {percent(data.contentStatus.draft, data.totals.contents)}%</p></div>
        </div>
      </article>
      ...
      <article className="overview-card overview-card-wide">
        <div className="overview-card-head"><span><TrendingUp size={18} /> 最近 7 天活跃趋势</span><small>内容 + 评论 + 联系</small></div>
        <TrendChart points={data.trend} />
      </article>

      <article className="overview-card overview-card-insight">
        <div className="overview-card-head"><span><Sparkles size={18} /> 数据分析建议</span><small><Heart size={13} /> {data.totals.likes} 点赞</small></div>
