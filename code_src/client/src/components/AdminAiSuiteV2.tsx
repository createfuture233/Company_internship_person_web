...
function LoadingLine({ label }: { label: string }) {
  return <div className="admin-ai-loading"><Loader2 className="admin-ai-spin" size={18} /><span>{label}</span><i></i></div>
}

export default function AdminAiSuiteV2() {
  ...
  const [generatePrompt, setGeneratePrompt] = useState('根据上传文件或当前说明，生成一篇适合个人网站发布的内容。')
  ...
  const [analysisPrompt, setAnalysisPrompt] = useState('请分析这篇内容还有哪些可以优化的地方。')
  ...
  function oneClickPrompt(type: ContentType) {
    const target = type === 'article' ? '文章' : '作品详情页'
    return [
      `请基于上传文件内容，一键生成一篇完整的${target}。`,
      '要求：标题明确，摘要自然，正文内容丰富，表达适合个人网站。',
      type === 'project' ? '如果是作品，请突出技术栈、功能亮点、实现过程和后续优化方向。' : '如果是文章，请突出背景、过程、思考和复盘价值。',
  ...
  return <section className="admin-module admin-ai-module admin-ai-safe-zone">
    ...
    <div className="admin-ai-tabs">
      {tabs.map(({ key, label, icon: Icon }) => <button key={key} className={active === key ? 'active' : ''} type="button" onClick={() => setActive(key)} disabled={Boolean(busy)}>
        <Icon size={17} />{label}
      </button>)}
    </div>
    ...
    {active === 'chat' && <div className="admin-ai-chat">
      <div className="admin-ai-messages">
        {!messages.length && <div className="admin-ai-empty"><Bot size={34} /><strong>问我一个后台问题</strong><span>例如：帮我规划下一篇文章、分析作品页、整理评论运营建议。</span></div>}
      ...
      <div className="admin-ai-suggestions">{['分析最近内容还缺什么', '帮我规划下一篇文章', '总结作品页面可以优化的地方'].map((item) => <button type="button" key={item} onClick={() => setChatInput(item)} disabled={Boolean(busy)}>{item}</button>)}</div>
      <form className="admin-ai-form" onSubmit={submitChat}>
        <textarea value={chatInput} maxLength={4000} rows={3} placeholder="向 AI 助手提问..." onChange={(event) => setChatInput(event.target.value)} />
    ...
    {active === 'upload' && <div className="admin-ai-card">
      ...
      {uploaded && <div className="admin-ai-oneclick">
        <button type="button" disabled={Boolean(busy)} onClick={() => generateDraft('article', oneClickPrompt('article'))}><Sparkles size={17} />一键生成文章</button>
        <button type="button" disabled={Boolean(busy)} onClick={() => generateDraft('project', oneClickPrompt('project'))}><Sparkles size={17} />一键生成作品</button>
      </div>}
    </div>}
