...
import { ArrowLeft, Edit3, ExternalLink, Eye, Image, Plus, Save, ShieldAlert, Tag, Trash2 } from 'lucide-react'
...
export default function AdminPanel({ contentType }: { contentType: ContentType }) {
  ...
  function editItem(item: ContentItem) {
    setForm(toForm(item)); setIsNew(false); setMode('edit'); setMessage('')
  }

  function createNew() {
    setIsNew(true); setForm(blankContent(contentType)); setMode('edit'); setMessage(`正在创建新的${typeName}，填写后点击保存。`)
  }

  function backToList() {
    setMode('list'); setIsNew(false); setForm(null); setMessage('')
  }
  ...
  async function save(event: { preventDefault: () => void }) {
    ...
    try {
      ...
      setForm(toForm(updated)); setIsNew(false)
  ...
  async function remove(item: ContentItem) {
    ...
    try {
      const response = await fetch(`${apiBase}/admin/content/${item.type}/${item.id}`, { method: 'DELETE', headers: headers() })
  ...
  if (message && !items.length && mode === 'list') return <section className="admin-panel"><ShieldAlert size={28} /><p>{message}</p></section>

  if (mode === 'list') return <section className="admin-panel admin-content-manager">
    <div className="admin-panel-head">
      <div><p className="eyebrow">{contentType === 'article' ? 'ARTICLE MANAGER' : 'PROJECT MANAGER'}</p><h1>{typeName}管理</h1><p>用表格管理{typeName}的发布状态、标签、更新时间与快捷操作。</p></div>
      <div className="admin-table-actions">
        <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as 'all' | ContentStatus)} aria-label="筛选状态">
    ...
    <div className="admin-table-wrap">
      <table className="admin-content-table">
        <thead><tr><th>{typeName}信息</th><th>状态</th><th>标签 / 技术栈</th><th>创建时间</th><th>上次修改</th><th>操作</th></tr></thead>
        <tbody>
          {visibleItems.map((item) => <tr key={item.id}>
            <td className="admin-table-main">
              <strong>{item.title}</strong>
              <span>{item.summary}</span>
              <small>ID: {item.id} · Slug: {item.slug} · {wordCount(item.body)} 字</small>
            </td>
            <td><span className={`admin-status admin-status-${item.status}`}>{statusText[item.status]}</span></td>
            <td className="admin-table-tags">
              {contentType === 'project' && item.stack && <em>{item.stack}</em>}
              <div>{item.tags.length ? item.tags.map((tag) => <span key={tag.id}>#{tag.name}</span>) : <small>暂无标签</small>}</div>
            </td>
            <td>{formatDate(item.createdAt)}</td>
            <td>{formatDate(item.updatedAt)}</td>
            <td>
              <div className="admin-table-row-actions">
                {item.status === 'published' && <a href={publicHref(item)} target="_blank" rel="noreferrer" title="查看真实页面"><ExternalLink size={16} /></a>}
                <button type="button" onClick={() => editItem(item)} title="编辑"><Edit3 size={16} /></button>
                <button className="admin-danger" type="button" onClick={() => remove(item)} title="删除"><Trash2 size={16} /></button>
              </div>
            </td>
          </tr>)}
        </tbody>
      </table>
      {!visibleItems.length && <p className="admin-state">暂无符合当前筛选条件的{typeName}。</p>}
    </div>
  </section>
  ...
  return <section className="admin-panel">
    <div className="admin-panel-head">
      <div><p className="eyebrow">{isNew ? 'CREATE CONTENT' : 'EDIT CONTENT'}</p><h1>{isNew ? `新增${typeName}` : `编辑${typeName}`}</h1><p>左侧修改字段，右侧实时预览详情页效果。</p></div>
      <div className="admin-table-actions">
        <button type="button" onClick={backToList}><ArrowLeft size={16} />返回列表</button>
