...
export default function AdminSidebar({ active }: { active: string }) {
  useEffect(() => {
    if (!localStorage.getItem('personal-planet-admin-token')) window.location.href = '/'
  }, [])
  return <aside className="admin-sidebar">
    <p className="eyebrow">ADMIN CONSOLE</p><h2>后台</h2><p>管理内容与站点信号。</p>
    <nav>{links.map(({ key, href, label, icon: Icon }) => <a href={href} className={active === key ? 'active' : ''} key={key}><Icon size={17} />{label}</a>)}</nav>
  </aside>
}